...
var _createClass = function () {
	...
	instance = !1,
	baseUrl = Cuteen_Config.ThemeUrl+"/libs/Get.php",
	CTPlayer = function () {
		function a(b) {
			var c = this;
		...
		return _createClass(a, [{
			...
			key: "template",
			value: function () {
				var a = '<audio class="CTPlayer-source" src="' + ("file" === this.type ? this.music[0].src : "") +
					'" preload="auto"></audio><div class="CTPlayer-picture"><img class="CTPlayer-cover" src="'
					+ this.music[0].cover + '" alt=""></div><div class="CTPlayer-control"><p class="CTPlayer-name">'
					+ this.music[0].name + '</p><div class="playController">' +
					'<div onclick="player.prev();" class="lastMusic  music-off ">' +
					'<svg class="icon" viewBox="0 0 1024 1024"><path d="M867.2 212.7L867.2 811.1c0 19.6-22.1 30.89999999-38 19.5l-415.9-299.2c-13.3-9.6-13.3-29.4 0-39L829.2 193.2c15.90000001-11.4 38-0.1 38 19.5z"  ></path><path d="M623.1 212.7L623.1 811.1c0 19.6-22.1 30.89999999-38 19.5l-415.90000001-299.2c-13.3-9.6-13.3-29.4 1e-8-39L585.1 193.2c15.90000001-11.4 38-0.1 38 19.5z"  ></path></svg></div>' +
					'<div  class="runMusic music-off btncon">' +
					...
					'<svg style="display:none" id="musicstopbtn" class="icon pause" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><path   d="M22.537 8.046h17.791c1.104 0 2.003.898 2.003 1.993v79.912a2.005 2.005 0 0 1-2.003 2.003h-17.79a2.005 2.005 0 0 1-2.003-2.003V10.04c0-1.095.898-1.993 2.002-1.993zM59.672 8.046h17.8c1.095 0 1.993.898 1.993 1.993v79.912a2.003 2.003 0 0 1-1.993 2.003h-17.8a1.997 1.997 0 0 1-1.993-2.003V10.04c0-1.095.889-1.993 1.993-1.993z"/></svg></div>' +
					'<div onclick="player.next();" class="nextMusic  music-off ">' +
					'<svg class="icon" viewBox="0 0 1024 1024"><path d="M156.8 811.3V212.9c0-19.6 22.1-30.9 38-19.5l415.9 299.2c13.3 9.6 13.3 29.4 0 39L194.8 830.8c-15.9 11.4-38 0.1-38-19.5z"  ></path><path d="M400.9 811.3V212.9c0-19.6 22.1-30.9 38-19.5l415.9 299.2c13.3 9.6 13.3 29.4 0 39L438.9 830.8c-15.9 11.4-38 0.1-38-19.5z"  ></path></svg></div><div style="left:95px" class="music-off CTPlayer-list-switch"><svg class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="3190" width="128" height="128"><path   d="M768 512c0-141.385-114.615-256-256-256S256 370.615 256 512s114.615 256 256 256 256-114.615 256-256z m-256 512C229.23 1024 0 794.77 0 512S229.23 0 512 0s512 229.23 512 512-229.23 512-512 512z m0-426.667c47.128 0 85.333-38.205 85.333-85.333S559.128 426.667 512 426.667 426.667 464.872 426.667 512s38.205 85.333 85.333 85.333z"  p-id="3191"></path></svg></div></div>' +
					'<p class="CTPlayer-author">' + this.music[0].author + '</p><div class="CTPlayer-percent"><div class="CTPlayer-line-loading"></div><div class="CTPlayer-line lter"></div></div><p class="CTPlayer-time"><span class="CTPlayer-cur">00:00</span>/<span class="CTPlayer-total">00:00</span></p><div class="CTPlayer-volume" style="' + (this.isMobile ? "display:none;" : "") + '"><i class="CTPlayer-icon glyphicon glyphicon-volume-up"></i><div class="CTPlayer-percent"><div class="CTPlayer-line"></div></div></div><i class="' + ("singleloop" === this.option.mode ? "CTPlayer-mode CTPlayer-mode-loop" : "CTPlayer-mode") + '"></i></div><ul class="CTPlayer-list animated flipInX">';
				for (var b in this.music) a += '<li data-index="' + b + '"><i class="CTPlayer-list-sign"></i><span class="CTPlayer-list-index">' + (parseInt(b) + 1) + '</span><span class="CTPlayer-list-name" title="' + this.music[b].name + '">' + this.music[b].name + '</span><span class="CTPlayer-list-author" title="' + this.music[b].author + '">' + this.music[b].author + "</span></li>";
				return a += "</ul>"
			}
		}, {
			key: "init",
			value: function () {
				...
				if (this.dom = {
					...
					timeline_total: this.root.querySelector(".CTPlayer-percent"),
					timeline_loaded: this.root.querySelector(".CTPlayer-line-loading"),
					timeline_played: this.root.querySelector(".CTPlayer-percent .CTPlayer-line"),
					timetext_total: this.root.querySelector(".CTPlayer-total"),
					timetext_played: this.root.querySelector(".CTPlayer-cur"),
					volumebutton: this.root.querySelector(".CTPlayer-icon"),
					volumeline_total: this.root.querySelector(".CTPlayer-volume .CTPlayer-percent"),
					volumeline_value: this.root.querySelector(".CTPlayer-volume .CTPlayer-line"),
					switchbutton: this.root.querySelector(".CTPlayer-list-switch"),
					modebutton: this.root.querySelector(".CTPlayer-mode"),
					musiclist: this.root.querySelector(".CTPlayer-list"),
					musicitem: this.root.querySelectorAll(".CTPlayer-list li")
				}, this.audio = this.root.querySelector(".CTPlayer-source"), this.option.listshow && (this.root.className = "CTPlayer-list-on"), "singleloop" === this.option.mode && (this.audio.loop = !0), this.dom.musicitem[0].className = "CTPlayer-curMusic", "file" === this.type) {
